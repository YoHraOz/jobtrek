...
const StorageManager = {
  ...
  async saveJobs(jobs) {
    return new Promise((resolve) => {
      chrome.storage.local.set({ [this.STORAGE_KEY]: jobs }, resolve);
    });
  },
  ...
  async addJob(job) {
    ...
    job.id = Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
  ...
  async updateJob(id, updates) {
    ...
    if (idx !== -1) {
      jobs[idx] = { ...jobs[idx], ...updates, dateModified: new Date().toISOString() };
      await this.saveJobs(jobs);
      return jobs[idx];
    }
    return null;
