...
(() => {
  ...
  const hasJobPath = /\/(jobs?|careers?|openings?|vacancies|positions)(\/|$|\?|#)/i.test(window.location.pathname);
  ...
  function addActionRow(value = '') {
    ...
    li.innerHTML = `
      ...
      <button class="jt-remove-action" title="Remove">×</button>
    `;
    li.querySelector('.jt-remove-action').addEventListener('click', () => li.remove());
    actionsList.appendChild(li);
  }

  // Bind the initial row's remove button
  actionsList.querySelector('.jt-remove-action').addEventListener('click', function() {
  ...
  function scrapeLinkedIn() {
    return {
      ...
      location: first(
        txt('.job-details-jobs-unified-top-card__primary-description-container .tvm__text'),
        txt('.job-details-jobs-unified-top-card__bullet'),
        txt('.topcard__flavor--bullet'),
        txt('.jobs-unified-top-card__bullet'),
        txt('.jobs-unified-top-card__subtitle .jobs-unified-top-card__subtitle-primary-grouping .t-black--light span'),
  ...
  function scrapeNaukri() {
    return {
      ...
      roleId: first(
        ...
        (() => {
          const labels = document.querySelectorAll('.styles_details__Y424J .styles_detail-row__RLBaK, .other-details .details-row');
  ...
  function scrapeGeneric() {
    ...
    const result = { company: '', position: '', location: '', roleId: '', salary: '' };

    if (ld) {
      // Company
      if (ld.hiringOrganization) {
        result.company = typeof ld.hiringOrganization === 'string'
      ...
      if (ld.jobLocation) {
        ...
        if (loc && loc.address) {
          const a = loc.address;
          result.location = [a.addressLocality, a.addressRegion, a.addressCountry]
            .filter(Boolean).join(', ');
        }
      }
      // Salary
      ...
      // Role ID / identifier
      if (ld.identifier) {
        result.roleId = typeof ld.identifier === 'string'
          ? ld.identifier
  ...
  function scrapeSalaryFromText() {
    ...
    const m = body.match(/[\$₹€£]\s?[\d,\.]+[\s]*[kK]?\s*[-–—to]+\s*[\$₹€£]?\s?[\d,\.]+[\s]*[kK]?(?:\s*(?:per|\/)\s*(?:year|annum|yr|month|mo))?/);
    if (m) return m[0].trim();
    // Also try "CTC: 10-20 LPA" pattern common on Indian platforms
    const m2 = body.match(/(?:CTC|salary|compensation|package)\s*[:\-–]?\s*([\d,\.]+\s*[-–to]+\s*[\d,\.]+\s*(?:LPA|lpa|Lacs?|Lakhs?|Cr|CPA|K|k)?)/i);
      ...
      try {
        ...
        if (el) {
          ...
          if (t.length > 2 && t.length < 100 && !/^location$/i.test(t)) {
            ...
            const cleaned = t.replace(/^(?:location|job\s*location|work\s*location)\s*[:\-–\n]\s*/i, '').trim();
            if (cleaned.length > 2) return cleaned;
          }
        }
      } catch (e) { /* selector may be invalid, skip */ }
    }
  ...
  function autoFill() {
    ...
    let scraped = { company: '', position: '', location: '', roleId: '', salary: '' };

    if (hostname.includes('linkedin.com')) {
      scraped = scrapeLinkedIn();
    } else if (hostname.includes('indeed.com')) {
      scraped = scrapeIndeed();
    } else if (hostname.includes('naukri.com') || hostname.includes('foundit.in')) {
    ...
    scraped.location = scraped.location
    ...
    if (!scraped.company && !JOB_PLATFORMS.some(d => hostname.includes(d))) {
      ...
      const cleanHost = hostname
        .replace(/^(www|jobs?|careers?|recruiting|hire|work|apply|talent|people|team|join|boards?)\./i, '')
        .replace(/^(www)\./i, '');
    ...
    if (!hidden.value) {
      ...
      for (const [domain, label] of Object.entries(platformMap)) {
        if (hostname.includes(domain)) {
          hidden.value = label;
          root.querySelectorAll('.jt-platform-chip').forEach(c => {
      ...
      if (!hidden.value) {
        ...
        root.querySelectorAll('.jt-platform-chip').forEach(c => {
          c.classList.toggle('jt-chip-selected', c.dataset.value === 'Company Website');
        });
      }
    }
  }
  ...
  function openPanel() {
    ...
    if (!comp.value) { comp.focus(); } else { $('#jt-position').focus(); }
  }
  ...
  function resetForm() {
    ['#jt-company','#jt-position','#jt-roleId','#jt-location','#jt-salary','#jt-applyThrough','#jt-url']
      .forEach(sel => { $(sel).value = ''; });
    $('#jt-status').value = 'Applied';
    root.querySelectorAll('.jt-platform-chip').forEach(c => c.classList.remove('jt-chip-selected'));
  ...
  function showToast(msg, isError) {
    ...
    toastEl.className = 'jt-toast jt-toast-show' + (isError ? ' jt-toast-err' : '');
    clearTimeout(toastEl._t);
    toastEl._t = setTimeout(() => { toastEl.className = 'jt-toast'; }, 2500);
  }
      ...
      chrome.storage.local.get(STORAGE_KEY, (result) => {
        ...
        chrome.storage.local.set({ [STORAGE_KEY]: jobs }, () => {
          if (chrome.runtime.lastError) {
            ...
            setTimeout(() => { closePanel(); resetForm(); }, 1000);
          }
        });
      });
    } catch (e) {
  ...
  cancelBtn.addEventListener('click', () => { closePanel(); resetForm(); });
  saveBtn.addEventListener('click', saveJob);
  overlay.addEventListener('click', (e) => { if (e.target === overlay) closePanel(); });
  document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && isOpen) closePanel(); });

  // Platform chips
  root.querySelectorAll('.jt-platform-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      const chips = root.querySelectorAll('.jt-platform-chip');
      const hidden = $('#jt-applyThrough');
  ...
  /* ---- Draggable FAB ---- */
  let dragging = false, dragOffsetX = 0, dragOffsetY = 0, hasMoved = false;
